...
const RecipeState = (props) => {
    ...
    useEffect(async () => {
        ...
        const dosa_results = data.data.hits;
        home_recipes.push(dosa_results[5])
        home_recipes.push(dosa_results[9])

        const data = await axios.get(`https://api.edamam.com/search?q=bhaji&app_id=${YOUR_APP_ID}&app_key=${YOUR_APP_KEY}`)
        const results = data.data.hits;
        home_recipes.push(results[5])
        home_recipes.push(results[9])
